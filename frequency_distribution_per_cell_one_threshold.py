...
def main():
    ...
    for filename in sorted(os.listdir(bax_structures_path)):  #sorted() makes that it loops over the files in the folder sorted
        if filename.endswith("cluster-areas.csv"):
            ...
            row_headers = ('relative frequency of small clusters < {})'.format(MEDIUM_THRESHOLD),
                           'relative frequency of large clusters >= {})'.format(MEDIUM_THRESHOLD), 'total cluster number')
